...
只需 Docker 和 Docker Compose，无需手动安装 MySQL、JDK、Node.js 或 Python。

项目提供了交互式配置脚本 `scripts/setup_docker.py`，按向导提示输入即可自动生成 `.env` 并启动整个 Docker 栈。

**前置条件：**
- 安装 [Docker Desktop](https://www.docker.com/products/docker-desktop/) 并确保守护进程已运行
- 准备好两个数据源 API Key：
  - **FRED API Key** — [申请地址](https://fred.stlouisfed.org/docs/api/api_key.html)
  - **EIA API Key** — [申请地址](https://www.eia.gov/opendata/register.php)
...
```
1. 端口配置        — 前端/后端/推理引擎/MySQL 对外端口（直接回车用默认值）
2. 数据库配置      — 数据库名、用户名、密码（留空自动生成随机密码）
3. 数据源配置      — FRED/EIA API Key、是否配置代理
4. 采集行为        — 启动时是否自动回填/采集
5. 大模型配置      — LLM API（不配置则使用 Mock 文本）
6. 启动确认        — 是否立即执行 docker compose up
```

脚本自动完成：生成 `.env` → 备份旧配置 → 启动 4 个容器（MySQL → Python 推理引擎 → Spring Boot 后端 → Nginx 前端）→ 建库建表。

**启动后访问：** http://localhost:5173（或你配置的前端端口）

**常用命令：**

```bash
docker compose logs -f              # 查看实时日志
docker compose logs python-engine   # 查看推理引擎日志
docker compose down                 # 停止所有服务
docker compose down -v              # 停止并清除数据卷（慎用，会丢失数据库数据）
docker compose up -d --build        # 重新构建并启动
python scripts/setup_docker.py      # 重新配置（自动备份旧 .env）
```
...
| 服务 | 镜像/构建 | 容器端口 | 说明 |
|------|-----------|----------|------|
| mysql | `mysql:8.0` | 3306 | 首次启动自动执行 `schema.sql` 建表 |
| python-engine | `docker/python-engine.Dockerfile` | 5000 | 等待 MySQL healthy 后启动 |
| backend | `docker/backend.Dockerfile` | 8080 | Maven 多阶段构建，JRE 17 运行 |
| frontend | `docker/frontend.Dockerfile` | 80 | Nginx 托管静态文件，反代 `/api/` 到后端 |
...
执行建表脚本（6 张表，`CREATE TABLE IF NOT EXISTS`，可重复执行）：
...
OilRisk_Alert/
├── docker/                             # Docker 构建文件
│   ├── backend.Dockerfile              # Java 后端（Maven 多阶段构建）
│   ├── frontend.Dockerfile             # 前端（Node 构建 + Nginx 托管）
│   ├── python-engine.Dockerfile        # Python 推理引擎
...
│   └── common/                         # 异常处理、统一响应
├── src/main/resources/
│   ├── application.properties          # 后端配置 ← 手动部署时需修改
│   ├── schema.sql                      # 建表脚本（Docker 自动执行）
│   └── mapper/*.xml
├── frontend/                           # Vue 前端
│   └── src/
│       ├── views/                      # 4 个页面
...
├── data/                               # 数据集
├── scripts/                            # 辅助脚本
│   └── setup_docker.py                 # 交互式一键配置向导（生成 .env + 启动容器）
└── docs/                               # 需求文档、设计文档
```
